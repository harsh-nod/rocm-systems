...
TEST_F(UBR_ConcurrentRegHang, SeparateBuffers_Grouped_EXPECTED_HANG)
{
    ...
    ASSERT_TRUE(sendInfo.allocated) << "Failed to allocate send buffers";
    ASSERT_TRUE(recvInfo.allocated) << "Failed to allocate recv buffers";

    // Initialize send buffers
    for (int peer = 0; peer < nRanks; peer++) {
        initializeBufferWithPattern<T>(sendInfo.buffers[peer], ELEMENTS_PER_PEER,
            [rank, peer](size_t) {
    ...
    ASSERT_EQ(ncclSuccess, ncclGroupEnd());  // THIS IS WHERE IT HANGS
...
TEST_F(UBR_ConcurrentRegHang, ContiguousBuffer_Grouped_SHOULD_WORK)
{
    ...
    ASSERT_TRUE(sendInfo.allocated) << "Failed to allocate contiguous send buffer";
    ASSERT_TRUE(recvInfo.allocated) << "Failed to allocate contiguous recv buffer";

    // Initialize send buffer views
    for (int peer = 0; peer < nRanks; peer++) {
        initializeBufferWithPattern<T>(sendInfo.views[peer], ELEMENTS_PER_PEER,
            [rank, peer](size_t) {
                return static_cast<T>(static_cast<float>(rank * 100 + peer));
            });
    }
...
TEST_F(UBR_ConcurrentRegHang, AllToAll_ContiguousBuffer_Stress_SHOULD_WORK)
{
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
        ...
        hipMemset(recvInfo.buffer, 0, totalSize);
    ...
    ASSERT_EQ(0, errors) << "Stress test failed with " << errors << " errors";
...
TEST_F(UBR_ConcurrentRegHang, ContiguousVsSeparate_Comparison_SHOULD_WORK)
{
    ...
    ASSERT_TRUE(sendContig.allocated) << "Failed to allocate contiguous send buffer";
    ASSERT_TRUE(recvContig.allocated) << "Failed to allocate contiguous recv buffer";
    ...
    // Initialize
    for (int peer = 0; peer < nRanks; peer++) {
        initializeBufferWithPattern<T>(sendContig.views[peer], ELEMENTS_PER_PEER,
            [rank, peer](size_t) {
                return static_cast<T>(static_cast<float>(rank * 100 + peer));
            });
    }
    ...
    ASSERT_TRUE(pattern1_ok) << "Contiguous buffer pattern failed verification";
    ...
    TEST_INFO("Rank %d: Fix: Always use contiguous buffers with views for grouped operations", rank);
}
...
TEST_F(UBR_ConcurrentRegHang, SeparateBuffers_NoRegistration_SHOULD_WORK)
{
    ...
    auto cleanup = makeScopeGuard([&]() {
        for (int i = 0; i < nRanks; i++) {
            if (sendBufs[i]) hipFree(sendBufs[i]);
            if (recvBufs[i]) hipFree(recvBufs[i]);
        }
    });

    for (int i = 0; i < nRanks; i++) {
        ASSERT_EQ(hipSuccess, hipMalloc(&sendBufs[i], bufSize));
        ASSERT_EQ(hipSuccess, hipMalloc(&recvBufs[i], bufSize));
    }

    // Initialize
    for (int peer = 0; peer < nRanks; peer++) {
        initializeBufferWithPattern<T>(sendBufs[peer], ELEMENTS_PER_PEER,
            [rank, peer](size_t) {
                return static_cast<T>(static_cast<float>(rank * 100 + peer));
            });
    }
...
protected:
    ...
    void cleanupBufferPair(RegInfo& sendInfo, RegInfo& recvInfo, ncclComm_t comm)
    {
        auto cleanup = [&](RegInfo& info) {
            ...
            if (info.buffer) { hipFree(info.buffer); info.buffer = nullptr; }
            info.registered = false;
        };
        cleanup(sendInfo);
        cleanup(recvInfo);
    ...
    // Initialize buffer with bfloat16 data
    void initBuffer(void* buffer, size_t countPerRank, int nRanks, int rank, int iter, float offset = 0.0f)
    {
        ...
        for (int dest = 0; dest < nRanks; dest++) {
            T value = static_cast<T>(computePattern(rank, dest, iter, offset));
            for (size_t i = 0; i < countPerRank; i++)
                data[dest * countPerRank + i] = value;
        }
        hipMemcpy(buffer, data.data(), data.size() * sizeof(T), hipMemcpyHostToDevice);
    }

    bool verifyBuffer(void* buffer, size_t countPerRank, int nRanks, int myRank, int iter, float offset = 0.0f)
    {
        std::vector<T> data(countPerRank * nRanks);
        hipMemcpy(data.data(), buffer, data.size() * sizeof(T), hipMemcpyDeviceToHost);
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            if (iter % 2 == 0) {
                ...
                if (iter < 2 && ctx.rank == 0) {
                    T sample;
                    hipMemcpy(&sample, sendInfo.buffer, sizeof(T), hipMemcpyDeviceToHost);
                ...
                if (iter < 2 && ctx.rank == 0) {
                    T recvSample;
                    hipMemcpy(&recvSample, recvInfo.buffer, sizeof(T), hipMemcpyDeviceToHost);
                ...
                if (iter < 3 && ctx.rank == 0) {
                    T sample;
                    hipMemcpy(&sample, sendInfoV.buffer, sizeof(T), hipMemcpyDeviceToHost);
                    TEST_INFO("[%s] iter=%d PRE-AllToAllv: sendBufV[0]=%.1f (expected=%.1f)",
                              label.c_str(), iter, static_cast<float>(sample),
                              computePattern(ctx.rank, 0, iter, 0.5f));
                }

                hipMemset(recvInfoV.buffer, 0, recvTotal * sizeof(T));
                ...
                if (iter < 3 && ctx.rank == 0) {
                    T recvSample;
                    hipMemcpy(&recvSample, recvInfoV.buffer, sizeof(T), hipMemcpyDeviceToHost);
...
TEST_F(UBR_AllToAllStress, InterleavedMultiSize_MultiNode)
{
    if (!setupMultiNode(RegTestConfig::MIN_RANKS_ALLTOALL, RegTestConfig::MIN_NODES_MULTINODE))
        GTEST_SKIP() << "Requires 4+ ranks across 2+ nodes";
    ASSERT_TRUE(isUBREnabled()) << "NCCL_LOCAL_REGISTER=1 required";
    ...
    ASSERT_TRUE(ok) << "Stress test failed";
}
...
TEST_F(UBR_AllToAllStress, InterleavedWithSplitComm_MultiNode)
{
    ...
    ASSERT_EQ(ncclSuccess, ncclCommSplit(primaryCtx.comm, color, primaryCtx.rank, &splitComm, nullptr));
    auto splitCleanup = makeScopeGuard([&]() { if (splitComm) ncclCommDestroy(splitComm); });
    ...
    ASSERT_TRUE(ok) << "Split comm stress test failed";
}
...
TEST_F(UBR_AllToAllStress, ConcurrentMultiComm_MultiNode)
{
    ...
    ASSERT_EQ(ncclSuccess, ncclCommSplit(primaryComm, 0, ctx1.rank, &comm2, nullptr));
    auto comm2Cleanup = makeScopeGuard([&]() { if (comm2) ncclCommDestroy(comm2); });

    hipStream_t stream1 = getActiveStream(), stream2;
    ASSERT_EQ(hipSuccess, hipStreamCreate(&stream2));
    auto stream2Cleanup = makeScopeGuard([&]() { hipStreamDestroy(stream2); });
    ...
    ASSERT_TRUE(allocateBufferPair(totalSize, primaryComm, send1, recv1));
    ASSERT_TRUE(allocateBufferPair(totalSize, comm2, send2, recv2));
    auto bufCleanup = makeScopeGuard([&]() {
    ...
    for (int iter = 0; iter < ITERATIONS_PER_SIZE; iter++) {
        ...
        hipMemsetAsync(recv1.buffer, 0, totalSize, stream1);
        hipMemsetAsync(recv2.buffer, 0, totalSize, stream2);
    ...
    ASSERT_EQ(0, errors) << "Concurrent multi-comm test failed with " << errors << " errors";
