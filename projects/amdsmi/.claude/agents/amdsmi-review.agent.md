...
**Library loading** (`amdsmi_wrapper.py`): Changes to `_detect_install_context`, `_build_candidate_paths`, or `_load_library` are **❌ BLOCKING** if they break system-package or pip-package context. Verify `Path(__file__).resolve()`, pip detection, `_libraries['libamd_smi.so']` dict key, and `tools/generator.py` sync.

**Header & wrapper consistency**: `amdsmi_wrapper.py` is generated by `tools/generator.py`. Public C API changes in `amdsmi.h` must propagate to wrapper + `amdsmi_interface.py`.

**API renames** must propagate to all layers: header → C impl → wrapper → interface → CLI → docs.
...
### Commit Messages
Prefer `[AMD-SMI]` or `[SWDEV-XXXXXX]` prefix tags.
