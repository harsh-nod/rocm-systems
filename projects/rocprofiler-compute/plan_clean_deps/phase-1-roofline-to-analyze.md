...
**Make `from roofline import Roofline` a lazy import**:
- **Delete** the top-level import `from roofline import Roofline` (line 38)
- **Move** it inside `analysis_setup()` so it only loads during analyze mode:
  ```python
  def analysis_setup(self, roofline_parameters):
      if roofline_parameters:
          from roofline import Roofline
          self.roofline_obj = Roofline(...)
  ```
- This is a 2-line change that genuinely eliminates plotly/dash/plotext/numpy/pandas from the profile import path, since the entire `roofline.py` module imports all of these at module level

**Clean up dead imports**: After deleting the PMC loading and post_processing code, the following top-level imports in `soc_base.py` become unused and should be removed (ruff will flag these):
- `from utils.file_io import create_df_pmc, load_profiling_config`
- `from utils.schema import Workload`
- `apply_filters` from `from utils.parser import BUILD_IN_VARS, SUPPORTED_DENOM, apply_filters`
- `impute_counters_iteration_multiplex`, `merge_counters_spatial_multiplex` from `from utils.utils import ...`
...
- **Add `analysis_setup()` call** before using roofline_obj (currently missing!):
  ...
  ```
- Add `hasattr(soc[gpu_arch], "roofline_obj")` guard before accessing roofline_obj (matches WebUI pattern)
...
- `test_analyze_generates_roofline_html` - Call profile binary handler (creates CSV), then call analyze binary handler (creates HTML), assert HTML files exist
- `test_analyze_roofline_multiple_datatypes` - Same two-step pattern with multiple data types
- `test_analyze_missing_roofline_csv_graceful` - Call analyze without prior profile, verify graceful failure
- `test_analyze_roofline_idempotent` - Run analyze twice, verify same output
- `test_analyze_corrupted_roofline_csv_graceful` - Write bad CSV, call analyze, verify graceful failure

**Important**: New analyze tests must invoke both `binary_handler_profile_rocprof_compute` and `binary_handler_analyze_rocprof_compute` to test the full workflow
...
2. `src/rocprof_compute_soc/soc_base.py` - Delete pmc check, PMC loading, deprecation, roofline_obj constructor creation, lazy Roofline import, dead import cleanup
